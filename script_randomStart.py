...
def normalize_with_ffmpeg():
    data = []
    for elt in os.listdir('./mp3'):
        if os.path.isdir('./mp3/'+elt):
            data.append(elt)
    for folder in data:
        for elt in os.listdir('./mp3/'+folder):
            if elt.endswith('.mp3'):
                # print(folder+"/"+elt)     
                cmd = "ffmpeg-normalize ./mp3/"+folder+"/"+elt+" -c:a libmp3lame -b:a 320k -o normalized/"+elt
                process = subprocess.Popen(cmd, stdout=subprocess.PIPE, shell=True)
                process.communicate()
...
def run_test(idx, config):
    ...
    print("playback_time:", testconfig["playback_time"], "TimeListening:", testconfig["TimeListening"], "distraction:", testconfig["Distractions"], "AudioLevel", testconfig["AudioLevel"], "NoiseLevel", testconfig["NoiseLevel"], "OuputLevel", testconfig["OuputLevel"])
    successful = run_test_with_config(testconfig)
    
    testconfig["successful"] = successful
...
def main():
    ...
    print(df.to_csv("result.csv", csep='\t', encoding='utf-8'))
